...
public class VipServlet extends HttpServlet{
	...
	//验证已购买商品
		...
		if (customer != null) {
			try {
				List<Order_details> list = new BoughtServiceImpl().selectBought1(customer.getCid());
				request.setAttribute("list", list);
		...
		if (error.size() > 0) {
			request.setAttribute("error", error);
			request.getRequestDispatcher("vip?method=seladdress").forward(request, response);
			return;
		}
		...
		}else if("seladdress".equalsIgnoreCase(method)) {
		...
		else if("deladdress".equalsIgnoreCase(method)) {
			Shipping_address shipping_address=new Shipping_address();
		...
		if(id !=null) {
			...
			try {
			new Shipping_addressServiceImpl().deleteShipping_address(id1);
		} catch (Exception e) {
			e.printStackTrace();
		}
	  }
